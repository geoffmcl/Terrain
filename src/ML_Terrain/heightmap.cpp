...
bool CMultiLayeredHeightmap::LoadTerrainShaderProgram(LPVOID lpParam)
{
    string msg, dir, file;
    if (lpParam) {
    ...
    file = dir;
    file += "data\\shaders\\terrain.vert";
	if (!shShaders[0].LoadShader(file, GL_VERTEX_SHADER))
    {
        msg = "Failed to load ";
        msg += file;
    ...
    file = dir;
    file += "data\\shaders\\terrain.frag";
	if (!shShaders[1].LoadShader(file, GL_FRAGMENT_SHADER))
    {
        msg = "Failed to load ";
        msg += file;
        MessageBox(NULL,msg.c_str(), "Critical Error", MB_ICONINFORMATION | MB_OK);
    ...
    file = dir;
    file += "data\\shaders\\dirLight.frag";
	if (!shShaders[2].LoadShader(file, GL_FRAGMENT_SHADER))
    {
        msg = "Failed to load ";
        msg += file;
        MessageBox(NULL,msg.c_str(), "Critical Error", MB_ICONINFORMATION | MB_OK);
