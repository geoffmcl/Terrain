...
bool COpenGLControl::InitOpenGL(HINSTANCE hInstance, HWND* a_hWnd, int iMajorVersion, int iMinorVersion,
    void (*a_ptrInitScene)(LPVOID), void (*a_ptrRenderScene)(LPVOID), void(*a_ptrReleaseScene)(LPVOID), LPVOID lpParam)
{
	if(!InitGLEW(hInstance)) 
    {
        MessageBox(NULL,"Failed InitGLEW!", "Critical Error", MB_ICONINFORMATION | MB_OK);
        return false;
    }
#ifdef FREEIMAGE_LIB
	...
	if(iMajorVersion <= 2)
	{
		...
		if (iPixelFormat == 0) 
        {
            MessageBox(NULL,"Failed ChoosePixelFormat!", "Critical Error", MB_ICONINFORMATION | MB_OK);
#ifdef FREEIMAGE_LIB
        ...
        {
            MessageBox(NULL,"Failed SetPixelFormat!", "Critical Error", MB_ICONINFORMATION | MB_OK);
            return false;
        }
	...
	if(ptrInitScene != NULL)
        ptrInitScene(lpParam);
