...
bool PrepareShaderPrograms(LPVOID lParam)
{
	...
	FOR(i, NUMSHADERS)
	{
        ...
        string file;
        if (lParam) {
            COpenGLControl *ogl = (COpenGLControl *)lParam;
            if (ogl->root) {
                file = ogl->root;
    ...
    {
        MessageBox(NULL,"Failed spMain.linkProgram!", "Critical Error", MB_ICONINFORMATION | MB_OK);
        return false;
    }
