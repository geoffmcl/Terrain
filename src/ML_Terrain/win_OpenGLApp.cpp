...
static bool ask_for_full = false;
...
bool COpenGLWinApp::RegisterAppClass(HINSTANCE a_hInstance)
{
	WNDCLASSEX wcex;
	...
	if (!RegisterClassEx(&wcex)) {
        MessageBox(NULL,"Failed to register class!", "Critical Error", MB_ICONINFORMATION | MB_OK);
        return false;
    }
    return true;
}
...
bool COpenGLWinApp::CreateAppWindow(string sTitle, LPSTR sCmdLine)
{
    if (sCmdLine && strlen(sCmdLine))
    {
        if (is_file_or_directory(sCmdLine) != MDT_DIR)
        {
            string msg = "Problem: Only input on the command line\r\n"
                "is a root path to the program data!\r\n"
                "Can NOT stat '";
            msg += sCmdLine;
            msg += "' as a valid directory! Aborting!";
            MessageBox(NULL,msg.c_str(), "Command Error", MB_ICONINFORMATION | MB_OK);
            return false;
        }
        oglControl.root = sCmdLine;
    }
	if(ask_for_full &&
        (MessageBox(NULL, "Would you like to run in fullscreen?", "Fullscreen", 
    ...
    if (!hWnd) {
        MessageBox(NULL,"Failed to create window!", "Critical Error", MB_ICONINFORMATION | MB_OK);
        return false;
    }
	if( !oglControl.InitOpenGL(hInstance, &hWnd, 3, 3, InitScene, RenderScene, ReleaseScene, &oglControl)) 
    {
        MessageBox(NULL,"Failed InitOpenGL!", "Critical Error", MB_ICONINFORMATION | MB_OK);
        return false;
    }
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR sCmdLine, int iShow)
{
	if(!appMain.InitializeApp("21_opengl_3_3"))
		return 0;
	if (!appMain.RegisterAppClass(hInstance))
        return 0;

	if(!appMain.CreateAppWindow("21.) Multilayered Terrain - Tutorial by Michal Bubnar (www.mbsoftworks.sk)", sCmdLine))
		return 0;
	appMain.ResetTimer();
