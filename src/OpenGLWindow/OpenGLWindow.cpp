...
#ifndef DEF_TERR2_BMP
#define DEF_TERR2_BMP "Terrain2.bmp"
#endif
////// Static Global Variables

static const char *terr_bmp = DEF_TERR2_BMP;

static HDC g_HDC;				// global device context
static bool fullScreen = false;	// true = fullscreen; false = windowed
static bool keyPressed[256];	// holds true for keys that are pressed	

static float angle       = 0.0f;	// camera angle 
static float radians     = 0.0f;	// camera angle in radians

////// Mouse/Camera Variables
static int mouseX = 0, mouseY = 0;						// mouse coordinates
static float cameraX = 0.0f, cameraY = 0.0f, cameraZ = 0.0f;	   // camera coordinates
static float lookX = 0.0f, lookY = 0.0f, lookZ = 0.0f;			   // camera look-at coordinates
...
static int Initialize()
{
	...
	imageData = LoadBitmapFile((char *)terr_bmp, &bitmapInfoHeader);
    if (!imageData) {
        MessageBox(NULL,"Failed to read BMP!", "Critical Error", MB_OK);
        return 0;
    }
	// initialize the terrain data and load the textures
	InitializeTerrain();
	if (!LoadTextures()) {
        MessageBox(NULL,"Failed to load texture!", "Critical Error", MB_OK);
        return 0;
    }
    return 1;
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	if (!Initialize()) {	   // initialize OpenGL
        done = true;
    }
